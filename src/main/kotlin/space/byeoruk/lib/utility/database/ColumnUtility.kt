package space.byeoruk.lib.utility.database

import space.byeoruk.lib.model.database.SchemaColumn
import java.sql.Connection
import java.sql.SQLException

object ColumnUtility {
    /**
     * 해당 스키마의 컬럼을 동기화 한다.
     *
     * @param connection DB 커넥션
     * @param table 테이블 이름
     * @param columns 정의된 Enum 컬럼들
     * @param useSqlite SQLite 사용 여부
     * @return 동기화 후 추가된 컬럼이 있으면 추가 컬럼 이름을 리스트 형태로 반환
     * @throws SQLException 테이블을 찾지 못한 경우. `CREATE TABLE` 직후에 호출하는 것을 전제로 하므로,
     *  이 경우는 설정이 잘못된 상태로 보고 조용히 넘어가지 않는다
     */
    fun syncColumns(connection: Connection, table: String, columns: List<SchemaColumn>, useSqlite: Boolean): List<String> {
        //  실제 DB의 컬럼 이름 모으기
        ...
        val missing = columns.filterNot { it.column().lowercase() in existing }
